...
CONFIG_PKG=github.com/home-ai-union/homeocto/pkg/config
...
## build-launcher: Build the homeocto-launcher (web console) binary
build-launcher:
	@echo "Building homeocto-launcher for $(PLATFORM)/$(ARCH)..."
ifeq ($(OS),Windows_NT)
	@$(POWERSHELL) "New-Item -ItemType Directory -Force -Path '$(BUILD_DIR)' | Out-Null"
	@$(MAKE) -C web build PLATFORM="$(PLATFORM)" ARCH="$(ARCH)" EXT="$(EXT)" OUTPUT="$(CURDIR)/$(BUILD_DIR)/homeocto-launcher-$(PLATFORM)-$(ARCH)$(EXT)" GO_BUILD_TAGS="$(GO_BUILD_TAGS)"
	@$(POWERSHELL) "Copy-Item -LiteralPath '$(BUILD_DIR)/homeocto-launcher-$(PLATFORM)-$(ARCH)$(EXT)' -Destination '$(BUILD_DIR)/homeocto-launcher$(EXT)' -Force"
else
	@mkdir -p $(BUILD_DIR)
	@GOARCH=${ARCH} $(MAKE) -C web build \
		OUTPUT="$(CURDIR)/$(BUILD_DIR)/homeocto-launcher-$(PLATFORM)-$(ARCH)$(EXT)" \
		WEB_GO='$(WEB_GO)' \
		GO_BUILD_TAGS='$(GO_BUILD_TAGS)' \
		LDFLAGS='$(LDFLAGS)'
	@$(LNCMD) homeocto-launcher-$(PLATFORM)-$(ARCH)$(EXT) $(BUILD_DIR)/homeocto-launcher$(EXT)
endif
	@echo "Build complete: $(BUILD_DIR)/homeocto-launcher$(EXT)"

build-launcher-frontend:
	@$(MAKE) -C web build-frontend

## build-launcher-tui: Build the homeocto-launcher TUI binary
build-launcher-tui:
	@echo "Building homeocto-launcher-tui for $(PLATFORM)/$(ARCH)..."
ifeq ($(OS),Windows_NT)
	@$(POWERSHELL) "New-Item -ItemType Directory -Force -Path '$(BUILD_DIR)' | Out-Null"
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/homeocto-launcher-tui-$(PLATFORM)-$(ARCH)$(EXT) ./cmd/homeocto-launcher-tui
	@$(POWERSHELL) "Copy-Item -LiteralPath '$(BUILD_DIR)/homeocto-launcher-tui-$(PLATFORM)-$(ARCH)$(EXT)' -Destination '$(BUILD_DIR)/homeocto-launcher-tui$(EXT)' -Force"
else
	@mkdir -p $(BUILD_DIR)
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/homeocto-launcher-tui-$(PLATFORM)-$(ARCH) ./cmd/homeocto-launcher-tui
	@ln -sf homeocto-launcher-tui-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/homeocto-launcher-tui
endif
	@echo "Build complete: $(BUILD_DIR)/homeocto-launcher-tui$(EXT)"
...
build-launcher-android-arm64:
	...
	@$(MAKE) -C web build-android-arm64 \
		...
		GO='$(GO)' \
		LDFLAGS='$(LDFLAGS)'
	@echo "Build complete: $(BUILD_DIR)/homeocto-launcher-android-arm64"
...
## build-macos-app: Build HomeOcto macOS .app bundle (no terminal window)
build-macos-app:build-launcher
	...
	@echo "macOS .app bundle created: $(BUILD_DIR)/HomeOcto Launcher.app"
